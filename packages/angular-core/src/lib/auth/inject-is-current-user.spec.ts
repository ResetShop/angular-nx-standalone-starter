...
import { injectIsCurrentUser } from './inject-is-current-user'

describe('injectIsCurrentUser', () => {
	...
	function createChecker() {
		return TestBed.runInInjectionContext(() => injectIsCurrentUser())
	}

	it('returns true when the entity id matches the current user id', () => {
		const isCurrentUser = createChecker()

		expect(isCurrentUser({ id: 42 })).toBe(true)
	})

	it('returns false when the entity id differs from the current user id', () => {
		const isCurrentUser = createChecker()

		expect(isCurrentUser({ id: 99 })).toBe(false)
	})
	...
	it('returns false when the current user is null', () => {
		...
		expect(isCurrentUser({ id: 42 })).toBe(false)
	})
	...
	it('re-evaluates inside a computed when the current user signal updates', () => {
		const isCurrentUser = createChecker()
		const target = { id: 7 }
		const matches = TestBed.runInInjectionContext(() => computed(() => isCurrentUser(target)))
