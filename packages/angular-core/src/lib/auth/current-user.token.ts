 ...
 * auth store under `CURRENT_USER_SOURCE` to make `injectIsCurrentUser()` and
 * `*ifNotCurrentUser` resolve.
 *
...
/**
 * DI token an app's auth store binds to (via `useExisting`) so cross-app
 * primitives like `injectIsCurrentUser()` and `IfNotCurrentUserDirective`
 * can read the current user without compile-time knowledge of the store.
 */
