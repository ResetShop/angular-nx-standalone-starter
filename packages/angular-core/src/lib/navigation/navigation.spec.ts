...
import { Navigation, NAVIGATION_PERMISSION_CHECK } from './navigation'
...
describe('Navigation Provider', () => {
	...
	beforeEach(async () => {
		TestBed.configureTestingModule({
			providers: [provideRouter(routes), { provide: NAVIGATION_CONFIG, useValue: mockNavigationConfig }],
		})
...
describe('Navigation Permission Filtering', () => {
	...
	function createService(config: NavigationConfig, permissionCheck?: (permission: string) => boolean): Navigation {
		TestBed.configureTestingModule({
			providers: [
				provideRouter([]),
				{ provide: NAVIGATION_CONFIG, useValue: config },
				...(permissionCheck ? [{ provide: NAVIGATION_PERMISSION_CHECK, useValue: permissionCheck }] : []),
			],
		})
	...
	it('should hide routes when user lacks the required permission', () => {
		...
		// Permission check returns false — lacks all permissions
	...
	it('should remove parent route when all children are filtered out', () => {
		...
		// All children filtered, parent should be removed, section empty
		expect(service.sections()).toHaveLength(0)
	})
