...
describe('UserMenu', () => {
	let openAccount: MockFn
	let openSettings: MockFn
	let logOut: MockFn
	...
	async function renderMenu(
		overrides: { collapsed?: boolean; actions?: RowActionsInput; placement?: NgpMenuPlacement } = {},
	) {
	...
	describe('menu', () => {
		...
		it('lists every action as a menu item, in order', async () => {
			const user = userEvent.setup()
			await renderMenu()
		...
		it('separates the header from the actions and each group from the next', async () => {
			const user = userEvent.setup()
			await renderMenu()
		...
		it('renders no separator for an empty group', async () => {
			const user = userEvent.setup()
			await renderMenu({ actions: [[{ label: 'Account', onSelect: openAccount }], []] })
		...
		it('accepts a flat list of actions as a single group', async () => {
			const user = userEvent.setup()
			await renderMenu({ actions: actions.flat() })
		...
		it('runs only the selected action and closes the menu', async () => {
			const user = userEvent.setup()
			await renderMenu()
			...
			expect(openAccount.calls).toHaveLength(0)
			expect(openSettings.calls).toHaveLength(0)
