...
- **`package.json` `build` / `test:e2e` / `storybook:build` scripts** — added the missing `--skip-nx-cache` flag so they bypass Nx Cloud, matching every other script in the file (`test`, `lint`, `stylelint`, `typecheck`, `test:integration`). Pre-branch the bypass came from the workflow appending `--skip-nx-cache` to each `npx nx ...` invocation directly; when those calls were converted to `npm run <task>` in this PR, three scripts didn't carry the flag and CI started failing with `Nx Cloud: Workspace is unable to be authorized. Exiting run.` ([#319](https://github.com/ResetShop/angular-nx-standalone-starter/issues/319))
