...
- **`README.md` Project Setup Guide** — added a "Running Tests" section documenting the zero-config integration-test workflow (real Postgres 17 spawned by `embedded-postgres` on a free localhost port), CI parity (same `pg.Pool`-backed `node-postgres` driver), and the `PG_TEST_CONNECTION_STRING` opt-in for long-lived databases. ([#321](https://github.com/ResetShop/angular-nx-standalone-starter/issues/321))
- **Local integration tests now run zero-config against a real, embedded Postgres 17 cluster.** When `PG_TEST_CONNECTION_STRING` is unset, `apps/reference-app/src/api/integration/setup/global-setup.ts` (vitest globalSetup) spawns a real `postgres:17`-equivalent process via the [`embedded-postgres`](https://www.npmjs.com/package/embedded-postgres) package on a free localhost port, pushes the curated schema with `drizzle-kit/api`, seeds base data, and tears the cluster down at suite end. The same `drizzle-orm/node-postgres` driver and `pg.Pool` are used everywhere — production, CI's `postgres:17` service container, and the local embedded path — so semantics match (real isolation levels, real pool concurrency, real LISTEN/NOTIFY, etc.). On a fresh `npm install` the EnterpriseDB binaries (~70 MB) are downloaded once and cached in `node_modules`. `apps/reference-app/src/api/integration/setup/embedded-pg-test-db.ts` owns the `EmbeddedPostgres` lifecycle (free-port allocation, init + start + createDatabase + stop). `apps/reference-app/vitest.integration.config.ts` registers `globalSetup` and keeps `fileParallelism: false` + `maxWorkers: 1` because tests share seed data across files; the win from real Postgres is fidelity, not parallelism. The `apps/reference-app/tsconfig.app.json` exclude on `src/api/integration/**` stays so `drizzle-kit/api` and `embedded-postgres` (devDeps) do not leak into the prod build's type graph. **Forks that depended on `getTestConnectionString` or the cross-process `globalSetup` lifecycle should review their integration suite — schema push and seed are now in `global-setup.ts`'s `setup()`, run once before any worker forks, and the connection string is published via `process.env['PG_TEST_CONNECTION_STRING']` for workers to inherit.** ([#321](https://github.com/ResetShop/angular-nx-standalone-starter/issues/321))
- **`apps/reference-app/src/db/schema/all.ts`** — new curated `schema` value module that aggregates every table, relations group, and `pgEnum` from the `db/schema/*` files. Replaces near-identical inline `schema = { ... }` literals previously maintained in `drizzle-postgres-connector.ts` and `db-helpers.ts`. Adding or renaming a schema entity now happens in one place. Curated value export — not a `*` re-export — so it does not violate the no-barrel-files constraint. Note: includes the `pgEnum` exports (`userStatusEnum`, `routeTypeEnum`) which are required for `drizzle-kit/api`'s `pushSchema` DDL generation; production `drizzle()` query usage tolerates either presence or absence. ([#321](https://github.com/ResetShop/angular-nx-standalone-starter/issues/321))
- **`embedded-postgres` devDependency** added — ships official EnterpriseDB Postgres 17 binaries (downloaded once at `npm install` time) and is used by the integration test suite to spawn a real Postgres cluster locally with no Docker daemon required. ([#321](https://github.com/ResetShop/angular-nx-standalone-starter/issues/321))
- **`apps/reference-app/src/api/helpers/base.repository.ts`** — added `DbExecutor` type alias (`DrizzlePgConnector | <tx handle>`). Repository helpers that may run from inside a `db.transaction(async (tx) => ...)` block must accept a `DbExecutor` parameter and propagate the `tx` handle when called from a transactional context. Routing through `this.db` while a transaction is in progress wedges single-connection backends (PGlite); on pooled backends it silently grabs a different connection that cannot see the transaction's uncommitted writes — a latent correctness bug that produces stale reads. The pattern was applied to `attachRolesToUsers` in `user-management.repository.ts` (called from `create()` and `updateStatus()` inside transactions). ([#321](https://github.com/ResetShop/angular-nx-standalone-starter/issues/321))
