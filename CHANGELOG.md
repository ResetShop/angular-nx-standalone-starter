...
- **`scripts/pglite-smoke.ts`** + **`@electric-sql/pglite` devDependency** + **`smoke:pglite` npm script** — verification artifact for the PGlite-based zero-Docker integration test plan ([#321](https://github.com/ResetShop/angular-nx-standalone-starter/issues/321)). The script creates an in-process PGlite instance, runs `drizzle-kit/api`'s `pushSchema` against it, and asserts the resulting WASM-Postgres database has the expected tables, the `user_status` `pgEnum`, and foreign-key constraints, plus an insert/select roundtrip works. Used as the verification gate before refactoring the existing cross-process `globalSetup` flow; subsequent commits in this PR build on the assertion that `pushSchema` correctly handles PGlite as a target.
