...
- **`npm run ci` / `scripts/tsx-alias-smoke.ts`** — added a `ci:tsx-smoke` step that runs a dedicated smoke entrypoint via tsx with the app tsconfig. The entrypoint imports both `@contracts/user/user.constants` (the alias family that broke in this issue) and `@schema/user` (declared in the app tsconfig but not currently used by any tsx-invoked script — covered as defense-in-depth). Catches tsx alias-resolution regressions locally before they reach GitHub Actions. ([#319](https://github.com/ResetShop/angular-nx-standalone-starter/issues/319))
- **`.github/workflows/ci.yml`** — added a dedicated `tsx-smoke` job that runs `npm run ci:tsx-smoke`. The smoke step was previously only chained inside local `npm run ci`; GitHub Actions invokes per-task scripts directly and would have skipped the guard. The job now runs alongside `typecheck`, `lint`, etc. ([#319](https://github.com/ResetShop/angular-nx-standalone-starter/issues/319))
- **`CLAUDE.md` "Local CI Verification" section** — updated to describe `npm run ci` as three serial stages (Stage 0 `ci:tsx-smoke` → Batch 1 → Batch 2). Previous wording said "two parallel batches" and omitted the smoke step, leaving agents with an incorrect mental model of what runs first. ([#319](https://github.com/ResetShop/angular-nx-standalone-starter/issues/319))
