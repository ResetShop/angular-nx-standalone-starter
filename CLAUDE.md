...
- Do **not** register `provideToastConfig` (it would have to live at app root for the root-singleton manager to read it, leaking toast config onto every route). Presentation options (`placement`, `duration`, `dismissible`) are passed per `show()` by `ToastBridgeService`; container-only settings (`maxToasts`, `gap`, `zIndex`) use ng-primitives' defaults (`maxToasts` is already 3).
