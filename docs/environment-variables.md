...
The database entry-point scripts — `drizzle:create-migrations`, `drizzle:push-migrations`, `drizzle:seed`, and `sync:permissions` — and the migration runner's `drizzle.config.ts` all read `PG_CONNECTION_STRING` through the `dbEnv` proxy (`@config/db.env`). The `drizzle-kit` scripts run through `tsx --tsconfig apps/reference-app/tsconfig.json` so the `@config/*` alias resolves. If `PG_CONNECTION_STRING` is missing or empty when you run one of these scripts, the proxy prints a formatted `FATAL: Environment validation failed (db domain)` message and exits with code `1` **at boot** — before any Drizzle or Postgres connection is attempted — instead of failing several lines later with a confusing connection error. This is the most common first-time setup snag for `npm run drizzle:seed`: deliver `PG_CONNECTION_STRING` via one of the four mechanisms above before running it.
